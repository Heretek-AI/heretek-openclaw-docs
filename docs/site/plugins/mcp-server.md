...
4. [Usage](#usage)
5. [Exposed Resources](#exposed-resources)
6. [Exposed Tools](#exposed-tools)
7. [Exposed Prompts](#exposed-prompts)
...
The OpenClaw MCP Server provides standardized access to Heretek OpenClaw capabilities through the Model Context Protocol (MCP). It exposes:
...
This implementation allows external MCP clients (IDEs, agents, tools) to connect to and utilize OpenClaw capabilities.

---

## Features

- **MCP Server Implementation** - Full MCP protocol support
- **Resource Handlers** - Access to memory, knowledge, and skills
- **Tool Handlers** - Skill execution and memory operations
- **Prompt Handlers** - Templated agent interactions
...
```json
{
  "plugins": {
    "mcp-server": {
      "enabled": true,
      "path": "./plugins/openclaw-mcp-server",
      "config": {
        "skillsPath": "./skills",
        "memoryPath": "./memory",
...
| [Skills Documentation](../agents/overview.md) | Skills repository |
| [Memory Architecture](../architecture/overview.md) | Memory systems |
